...
const CashInHis = () => {
    ...
    const { data: items = [], isLoading } = useQuery({
        queryKey: ["cashInHistory"],
        enabled: !loading && !!user,
        queryFn: async () => {
            const { data } = await axiosSecure.get(`/cashinRequest?email=${user?.email}&phone=${user?.phone}`);
            return data;
        }
    })
    ...
    return (
        <div>
            <table className="w-full text-left border border-separate rounded bg-white border-slate-200" cellSpacing="0">
                <tbody>
                    <tr>
                        <th scope="col" className="hidden h-12 px-6 text-sm font-medium border-l sm:table-cell first:border-l-0 stroke-slate-700 text-slate-700 bg-slate-100">Agent Number</th>
