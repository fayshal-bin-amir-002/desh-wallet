...
const AdminCashInHis = () => {
    ...
    const { data: items = [], isLoading } = useQuery({
        queryKey: ["cashInHistoryAdmin"],
        enabled: !loading && !!user,
        queryFn: async () => {
            const { data } = await axiosSecure.get(`/AdminCashInHis?email=${user?.email}&phone=${user?.phone}`);
            return data;
        }
    })
